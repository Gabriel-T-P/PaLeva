...
describe 'visitante busca por um pedido' do
  ...
  it 'e esta com status em preparo' do
    allow(SecureRandom).to receive(:alphanumeric).and_return('AAA111')
    establishment = Establishment.create!(corporate_name: 'Carlos LTDA', trade_name: 'Carlos Café', full_address: 'Rio Branco, Deodoro', 
                                            cnpj: CNPJ.generate, email: 'carlosjonas@email.com', phone_number: '99999043113')
    user = User.create!(first_name: 'Carlos', last_name: 'Jonas', cpf: CPF.generate, email: 'carlosjonas@email.com', password: '1234567891011', establishment: establishment)
    ...
    fill_in 'E-mail',	with: 'sometext@email.com'
    click_on 'Salvar Pedido'
    patch set_status_cooking_api_v1_order_path(id: 1)
    visit display_order_path(code: 'AAAA1111')
    expect(page).to have_content "Em preparo #{I18n.l(DateTime.current)}"
  end
  
  it 'e esta com status pronto' do
    allow(SecureRandom).to receive(:alphanumeric).and_return('AAA111')
    establishment = Establishment.create!(corporate_name: 'Carlos LTDA', trade_name: 'Carlos Café', full_address: 'Rio Branco, Deodoro', 
                                            cnpj: CNPJ.generate, email: 'carlosjonas@email.com', phone_number: '99999043113')
    user = User.create!(first_name: 'Carlos', last_name: 'Jonas', cpf: CPF.generate, email: 'carlosjonas@email.com', password: '1234567891011', establishment: establishment)
    ...
    fill_in 'E-mail',	with: 'sometext@email.com'
    click_on 'Salvar Pedido'
    patch set_status_ready_api_v1_order_path(id: 1)
    visit display_order_path(code: 'AAAA1111')
    expect(page).to have_content "Pronto #{I18n.l(DateTime.current)}"
  end
end
